...
CONTAINS
   ...
   SUBROUTINE tra_mle_trp( kt, kit000, pu, pv, pw, puu, pvv, cdtype, Kmm )
      !!----------------------------------------------------------------------
      !!                  ***  ROUTINE tra_mle_trp  ***
      !!
      ...
      REAL(dp), DIMENSION(jpi,jpj,jpk), INTENT(inout) ::   puu, pvv    ! ocean velocity field and RHS of momentum equation
      !
      ...
      REAL(wp), DIMENSION(A2D(nn_hls))     :: zpsim_u, zpsim_v, zmld, zbm, ztm, zsm, zhu, zhv, zn2, zLf_NH, zLf_MH
      REAL(wp), DIMENSION(A2D(nn_hls),jpk) :: zpsi_uw, zpsi_vw
      REAL(wp), DIMENSION(A2D(nn_hls),jpk) :: sis_w, sin_w
      REAL(wp), DIMENSION(A2D(nn_hls))     :: dbu, dbv, dtu, dtv, dsu, dsv
      REAL(wp), DIMENSION(A2D(nn_hls))     :: sigms, sigmn, sigm2
      !!----------------------------------------------------------------------
      !
      !
      ...
      ELSE !do not use osn_mle
         ...
         DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, ikmax )                    ! MLD and mean buoyancy and N2 over the mixed layer
            ...
            ztm (ji,jj) = ztm (ji,jj) + zc * pts(ji,jj,jk,jp_tem)
            zsm (ji,jj) = zsm (ji,jj) + zc * pts(ji,jj,jk,jp_sal)
         ...
         DO_2D( nn_hls, nn_hls, nn_hls, nn_hls )
            ...
            ztm(ji,jj) = ztm(ji,jj) / MAX( e3t(ji,jj,1,Kmm), zmld(ji,jj) )
            zsm(ji,jj) = zsm(ji,jj) / MAX( e3t(ji,jj,1,Kmm), zmld(ji,jj) )
         END_2D
         !
         !
         ...
         ELSEIF( nn_mle == 1 ) THEN       ! New formulation (Lf = 5km fo/ff with fo=Coriolis parameter at latitude rn_lat)
            ...
            END_2D
         ELSEIF( nn_mle == 2 ) THEN      ! Strain-induced Zhang et al. 2025
            DO_2D( nn_hls, nn_hls-1, nn_hls, nn_hls-1 )
               dbu(ji,jj) = zbm(ji+1,jj) - zbm(ji,jj)
               dbv(ji,jj) = zbm(ji,jj+1) - zbm(ji,jj)
               dtu(ji,jj) = ztm(ji+1,jj) - ztm(ji,jj)
               dtv(ji,jj) = ztm(ji,jj+1) - ztm(ji,jj)
               dsu(ji,jj) = zsm(ji+1,jj) - zsm(ji,jj)
               dsv(ji,jj) = zsm(ji,jj+1) - zsm(ji,jj)
            END_2D
         ENDIF
