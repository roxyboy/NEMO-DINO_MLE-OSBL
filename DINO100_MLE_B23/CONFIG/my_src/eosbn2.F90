MODULE eosbn2
   ...
   ! REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   pabT  ! thermo expansion coeff.
...
CONTAINS
   ...
   SUBROUTINE rab_3d_t( pts, ktts, pab, ktab, Kmm )
      ...
      REAL(wp), DIMENSION(SIZE(pab,1),SIZE(pab,2),jpk) ::   pabT
      !
      INTEGER  ::   ji, jj, jk                ! dummy loop indices
      ...
      CASE( np_teos10, np_eos80 )                !==  polynomial TEOS-10 / EOS-80 ==!
         ...
         DO_3D( nn_hls, nn_hls, nn_hls, nn_hls, 1, jpkm1 )
            ...
            zn  = ( ( zn3 * zh + zn2 ) * zh + zn1 ) * zh + zn0
            !
            pabT(ji,jj,jk) = zn * r1_rho0 * ztm
            ! pab(ji,jj,jk,jp_tem) = zn * r1_rho0 * ztm
            pab(ji,jj,jk,jp_tem) = pabT(ji,jj,jk)
            !
            ! beta
            zn3 = BET003
            !
