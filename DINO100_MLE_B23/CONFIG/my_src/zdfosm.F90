MODULE zdfosm
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::   swb0        ! Surface buoyancy flux
...
CONTAINS
   ...
   SUBROUTINE zdf_osm( kt, Kbb, Kmm, Krhs, p_avm,   &
      ...
      IF ( ln_dia_osm ) THEN
         ...
         !CALL zdf_osm_iomput( "zwb0tot",         tmask(A2D(0),1) * zwb0tot(A2D(0)) )        ! Total surface buoyancy flux including insolation
      END IF
      !
